...
static tFileStatus tryReadingFile(FIL *fil, void *buffer, UINT btr, UINT *br){
	...
	//Try to open the file
	do{
		fresult = f_read(fil, buffer, btr, br);
		attemps++;
	}while(fresult != FR_OK && attemps < FILE_MAX_RETRIES);

	//Return operation result
	return (fresult == FR_OK) ? FILE_OK : FILE_ERROR_READ;
}

static tFileStatus tryWritingFile(FIL *fil, void *buffer, UINT btw, UINT *bw){
	uint8_t attemps = 0;

	//Try to open the file
	do{
		fresult = f_write(fil, buffer, btw, bw);
		attemps++;
	}while(fresult != FR_OK && attemps < FILE_MAX_RETRIES);

	//Return operation result
	return (fresult == FR_OK) ? FILE_OK : FILE_ERROR_WRITE;
}
